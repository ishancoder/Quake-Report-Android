...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //Get the JSONArray of all the earthquakes
            JSONArray earthquakeArray = earthquakeObject.getJSONArray("features");

            for(int i=0 ; i < earthquakeArray.length(); i++){
                JSONObject object = earthquakeArray.getJSONObject(i).getJSONObject("properties");
                double mag = object.getDouble("mag");
                String place = object.getString("place");
                long time = object.getLong("time");
                Uri url = Uri.parse(object.getString("url"));
                earthquakes.add(new Earthquake(place, mag, time, url));
            }

            throw new JSONException("HOLA!! I'm a exception!");
